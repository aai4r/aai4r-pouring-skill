...
def retrieve_cfg(args, use_rlg_config=False):
    if use_rlg_config:
        ...
        if args.task == "BallBalance":
            ...
            return os.path.join(args.logdir, "ingenuity"), "cfg/train/rlpt/pytorch_ppo_mirobot_cube.yaml", "cfg/mirobot_cube.yaml"
        else:
            warn_task_name()
...
def get_args(benchmark=False, use_rlg_config=False, target_task="Humanoid", headless=False):
    custom_parameters = [
        ...
        {"name": "--task", "type": str, "default": target_task,
    ...
    if use_rlg_config == False:
        if args.horovod:
            print("Distributed multi-gpu training with Horovod is not supported by rl_algos. Use rl_games for distributed training.")
        if args.steps_num != -1:
            print("Setting number of simulation steps per iteration from command line is not supported by rl_algos.")
        if args.minibatch_size != -1:
            print("Setting minibatch size from command line is not supported by rl_algos.")
        if args.checkpoint != "Base":
            raise ValueError("--checkpoint is not supported by rl_algos. Please use --resume <iteration number>")
